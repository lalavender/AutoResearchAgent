...
- 搜索：两级策略 — **Tier 1** LLM 内置搜索优先 → **Tier 2** Tavily 实时搜索（LangChain 集成）
...
```bash
uv sync                          # 安装依赖
uv run backend/main.py           # 启动服务（http://localhost:9000）
uv run pytest                    # 运行测试
uv add <package>                 # 添加依赖
uv add --dev <package>           # 添加开发依赖
```

> 注意：后端代码在 `backend/` 目录，启动时需指定完整路径
...
│   │   ├── search.py              # 两级搜索（Tier1 LLM + Tier2 Tavily）
│   │   ├── extract.py             # 从搜索结果提取关键发现
│   │   ├── gap_analysis.py        # 评估知识缺口
│   │   └── report.py              # 综合生成 Markdown 报告
│   ├── tools/
│   │   ├── __init__.py            # 导出工具列表
│   │   ├── tavily_search.py       # Tavily 搜索工具（LangChain 集成）
...
| `search` | 两级并发搜索（Tier1+Tier2 并行），每轮 10 个问题（可配置），每问题 5 个网页源 | flash | plan, completed | search_results |
...
│  │  │                                                      │   │  │
│  │  │ 【Tier 2】Tavily 搜索 + 内容获取                  │   │  │
│  │  │   ├─ tavily_search_with_content(query, max_results=5)             │   │  │
│  │  │   ├─      │   │  │
│  │  │   ├─ 内容截断至 8000 词（取前 8000 词）              │   │  │
│  │  │   └─ Tavily API Key 未配置时优雅跳过                         │   │  │
│  │  │                                                      │   │  │
│  │  │  Tier1 + Tier2 通过 asyncio.gather 并发执行           │   │  │
│  │  │                                                      │   │  │
│  │  │  缺口补充搜索（第2轮+，并行于主搜索）:                  │   │  │
│  │  │   └─ gap_query → tavily_search(max_results=3)      │   │  │
│  │  └──────────────────────────────────────────────────────┘   │  │
│  │                                                              │  │
│  │  并发控制: LLM Semaphore(4)            │  │
...
search_node(state)
  ...
  ├─ batch = open_questions[:MAX_QUESTIONS_PER_ROUND]  ← 每轮最多 20 个问题
  │
...
### 并发执行模型
- 每轮 10 个问题（可配置）通过 `asyncio.gather` 并行搜索
- 每个问题内部 Tier1（LLM）和 Tier2（Tavily）通过 `asyncio.gather` 同时启动
- **LLM 并发限制**：`asyncio.Semaphore(8)` 防止 API 限流
...
- `tavily_search_with_content(query, max_results=5)` → 获取搜索结果 + AI摘要 + 原始内容
- 内容截断至 8000 词（取前 8000 词）
- Tavily API Key 未配置时优雅跳过，仅使用 Tier1 结果

### 缺口补充搜索（第 2 轮+）
- gap_query → tavily_search(max_results=3)
- 结果标记 source=tavily_gap
...
4. **单层并发控制**：LLM 调用 `asyncio.Semaphore(4)` 防止 API 限流
...
7. **内容截断策略**：网页原始内容保留至 200000 字符，extract 阶段每个来源 snippet 截断至 50000 字符（前后各 25000），组合内容上限 80000 字符，report 阶段 findings 上限 200000 字符
...
| Tavily API Key 未配置 | 返回空列表，仅使用 Tier1 结果 |
...
| Tavily 限流 | 月额度 1000 次，免费额度用尽后仅使用 Tier1 |
...
6. **错误降级**：Tavily 不可用时验证 Tier1 独立完成，状态栏正确提示
